...
4. **Mental Model Mermaid required by issue, not by validator.** Issue #28 AC#2 says "mental-model Mermaid renders correctly", but `MERMAID_RULES.requiredIn` is `[]` and the live required-H2 contract has no Mental Model slot. Override: added an optional Mental Model Mermaid while leaving the 6 required validator sections intact. → Action: reconcile the issue template that produced #28's AC list with the live `template-contract.ts` — one of them should change so the spec is single-source.
